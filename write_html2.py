...
.topic-info{flex:1}
.topic-title{font-size:18px;font-weight:700;color:#fff;line-height:1.4}
.sources{display:flex;flex-wrap:wrap;gap:6px;margin-top:10px}
.source-pill{display:inline-flex;align-items:center;gap:4px;padding:4px 10px;background:#1a1a35;border:1px solid #333;border-radius:16px;font-size:11px;color:#ccc;text-decoration:none;transition:all .2s}
...
.video-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px;margin-top:12px}
.video-card{background:#0d0d20;border:1px solid #2a2a4a;border-radius:8px;overflow:hidden;text-decoration:none;color:inherit;display:block;transition:all .2s}
.video-card:hover{border-color:#fb7299;transform:translateY(-2px)}
...
.video-play{position:absolute;bottom:4px;right:4px;background:rgba(0,0,0,.85);color:#fff;font-size:10px;padding:2px 5px;border-radius:3px}
...
async function r1(){
if(!D||!D.hot_topics||D.hot_topics.length===0){
document.getElementById("c").innerHTML='<div class="empty">No data</div>';return
}
